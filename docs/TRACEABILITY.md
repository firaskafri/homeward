...
| ONB-004 | Disabled progression explains its blocker visibly and programmatically | `OnboardingView.swift`, shell onboarding journey | Implemented |
| PRE-001 | Preview is optional and offers explicit Run Preview and Skip Preview choices | `OnboardingView.swift`, shell onboarding-review journey | Implemented with separate Run Preview, Skip Preview, and Start Homeward actions |
...
| PRE-003 | Preview cancellation states that an accepted normal quit is irreversible | `PreviewView` | Implemented with explicit confirmation before ending an active preview |
| PRE-004 | Preview covers first exit, relaunch, second exit, completion, attention, timeout, and cancellation | `AppModel.PreviewState`, `PreviewView`, preview tests | Implemented with reason-specific missing, refusal, timeout, and exited-process guidance |
...
| SCH-005 | A change that makes the current time unavailable confirms the exact Gentle/Firm consequence | `ScheduleEditorView.swift`, `TodayView.swift`, `HomewardApp.swift` | Implemented with close-mode-specific confirmation copy |
| SCH-006 | Today-only overrides are bounded and Return to Weekly Schedule explains immediate closing when applicable | `Models.swift`, `ScheduleResolver.swift`, `AppModel.swift`, shell return-to-weekly journey | Implemented with immediate-close detection and confirmation |
...
| APP-005 | Zero apps blocks onboarding and becomes a post-setup attention state | `OnboardingView.swift`, `AppPickerView.swift`, `AppModel.attentionCount`, `completedSetupWithoutApplicationsNeedsWorkAppsAttention()` | Implemented |
...
| APP-008 | Duplicate names and multiple instances are visibly disambiguated with explicit action scope | `ApplicationCatalog.swift`, `ClosingPanel.swift`, `ApplicationListFormatterTests.swift` | Implemented with path disambiguation in selection and process-ID disambiguation in lifecycle actions |
...
| GNT-002 | Needs-attention offers Show App and per-process Leave Open This Time | `ClosingPanel.swift`, `AppModel.leaveOpen()`, Release lifecycle tests | Implemented |
...
| FRM-001 | Firm requests normal quit and gives each process a complete 30-second grace | `AppModel.beginEnforcement()`, `Enforcement.swift`, Release lifecycle tests | Implemented with deterministic and fixture-backed Release evidence |
...
| BLK-001 | Relaunched selected processes are closed according to current schedule and mode | `WorkspaceMonitor.swift`, `AppModel.handleLaunchSnapshot()`, Release lifecycle tests | Implemented with fixture-backed process relaunch evidence |
...
| BLK-003 | Process identity prevents a deadline or termination event from transferring to a relaunch/PID reuse | `ProcessSessionID`, `EnforcementPlanner.forceEligibleTargetIDs()`, `AppModel.terminationMatches()`, identity and Release lifecycle tests | Implemented |
...
| NTE-002 | Capture focuses the editor, prevents duplicate saves, and preserves the draft after failure | `NotesViews.swift`, `AppModel.saveNote()`, shell persistence-failure journey | Implemented with Saving and success states |
